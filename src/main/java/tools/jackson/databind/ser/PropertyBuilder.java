 ...
 * project depends on. The ONLY functional change vs the upstream class is in the
 * Include.NON_DEFAULT branch of buildWriter(): it does NOT unconditionally suppress
 * nulls when comparing against a real default-bean instance (Jackson [databind#1757]).
 * This restores the pre-3.2 behavior required here: a property explicitly set to
 * null is still written when its bean default is non-null, so that the null
 * round-trips and "unsets" the default on read-back. Properties whose default is
 * itself null are still suppressed (so output stays clean). Requires
 * MapperFeature.USE_REAL_INCLUDE_NON_DEFAULT to be enabled (see BeanMapper).
 */
/**
...
//                serializationType = TypeFactory.type(am.getGenericType(), _beanDesc.getType());
        ...
        case NON_DEFAULT:
            ...
            if (_useRealPropertyDefaults && (defaultBean = getDefaultBean()) != null) {
                ...
                // NORCONEX: upstream [databind#1757] adds "suppressNulls = true" here,
                // which suppresses an explicit null even when the bean default is
                // non-null. We intentionally omit that so such nulls are written and
                // round-trip (overriding the default on read). When the real default
                // is itself null, the valueToSuppress==null branch below still sets
                // suppressNulls=true, keeping output clean.
            } else {
